...
// Computing sequences from 0 Hz to a given value of spin frequency
 ...
 while(1){   
 //printf("---------------------------------------------------------------------------\n");
  ratio_r = ratio_r - 0.0025; //0.0025 for all eos except APR; for APR, 0.002

  for(j=0;j<3;j++){
   ej = temp_energy - 0.01*j; // 0.01 for all eos except APR; for APR, 0.005
 ...
 // Computing one neutron star
    ...
    printf("%g   %4.5f  %4.5f  %.5f %.3f  %.3f  %.5f\n",
